...
## Local Claude and Codex proxy setup

`setup-local-proxy.sh` configures a macOS or Linux developer machine to
run Claude Code and OpenAI Codex through the hardened local Headroom token proxy.

```bash
scripts/setup-local-proxy.sh --shell auto
```

The script creates `.venv-headroom`, installs `.[proxy]`, installs Claude Code
and OpenAI Codex with npm when they are missing, and writes a marked,
idempotent wrapper block to the selected shell rc file. The wrapper installs
`claude` / `tclaude` and `codex` / `tcodex`, and keeps
`HEADROOM_MODE=token`, `HEADROOM_TELEMETRY=off`, and
`HEADROOM_LOCAL_CONTROL_GUARD_ENABLED=true`.

To manage CLI installation yourself, opt out explicitly:

```bash
scripts/setup-local-proxy.sh --no-install-claude --no-install-codex
```
